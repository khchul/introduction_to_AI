...
def logic02_03():
    ...
    ################# Write Your Code Here #########################
    return Forall('$y', Forall('$x', Equiv(Daughter('$x','$y'), And(Female('$y'), Child('$x', '$y')))))
...
def logic02_04():
    ...
    ################# Write Your Code Here #########################
    return Forall('$y', Forall('$x', Equiv(Grandmother('$x', '$y'), Exists('$z',\
                                                                           And(Female('$y'), And(Parent('$x', '$z'),\
                                                                                                 Parent('$z', '$y')))))))
...
def suspect():
    ...
    formulas.append(Exists('$x', Forall('$y', And(TellTruth('$x'), Implies(Not(Equals('$x', '$y')),Not(TellTruth('$y')))))))
    formulas.append(Exists('$x', Forall('$y', And(CrashedServer('$x'), Implies(Not(Equals('$x', '$y')),Not(CrashedServer('$y')))))))
    ################################################################
    # Query: Who did it?
...
def number_theorem():
    ...
    query = None
    ################# Write Your Code Here #########################
    formulas.append(Forall('$x', Exists('$y', Forall('$z', And(And(Not(Equals('$x', '$y')), Successor('$x','$y')), Implies(Not(Equals('$y', '$z')), \
                                                                                            Not(Successor('$x', '$z'))))))))
